...
<?php
    ...
    //根据姓名从member_info_table_cui中查出member_id,card_id和card_type
    $query_result = $conn->query("select member_id,card_id,card_type from member_info_table_cui where member_name='".$name."'");
    debug_output("select member_id,card_id,card_type from member_info_table_cui where member_name='".$name."'");
    ...
    foreach($result as $value)  //member_info_table_cui 0:member_id 1:card_id 2:member_name
    {
        //根据查出的member_id到class_booking_table_cui中查出此用户已选的课程
        $query = $conn->query("select * from class_booking_table_cui where member_id=$value[0] order by class_id");
        debug_output("select * from class_booking_table_cui where member_id=$value[0] order by class_id");
        $class_booking_result = $query->fetchAll();

        foreach($class_booking_result as $class_booking_value)  //class_booking_table_cui 0:class_id 1:member_id 2:waiting_No
        {
            //到class_info_table_cui表中查找出课程名称和课程时间
            $query = $conn->query("select * from class_info_table_cui where class_id=$class_booking_value[0]");
            debug_output("select * from class_info_table_cui where class_id=$class_booking_value[0]");
            $class_info_result = $query->fetchAll();

            foreach($class_info_result as $class_info_value)    //class_info_table_cui 1:class_name 2:begin_time 
            {
