...
//    }
    ...
    //到class_info_table中查出max_user_number,selected_user_number
    ...
    $query_result = $conn->query("select waiting_No from class_booking_table where member_id=$db_member_id and class_id=$input_class_id and canceled=0");
    debug_output("select waiting_No from class_booking_table where member_id=$db_member_id and class_id=$input_class_id and canceled=0");
    ...
    if($db_card_type == TIME_CARD_TYPE) //计时卡
    {
        ...
        {
            debug_output("计时卡");
            $query_result = $conn->query("select used_times,next_used_times from time_card_table where card_id='".$input_card_id."'");
            debug_output("select used_times,next_used_times from time_card_table where card_id='".$input_card_id."'");
            $result = $query_result->fetch();
    ...
    {
        debug_output("计次卡");
        $query_result = $conn->query("select card_status,used_times,max_times from measured_card_table where card_id='".$input_card_id."'");
        debug_output("select card_status,used_times,max_times from measured_card_table where card_id='".$input_card_id."'");
        $result = $query_result->fetch();
        ...
        {
            $query_result = $conn->query("update measured_card_table set card_status=1,used_times=$db_used_times where card_id='".$input_card_id."'");
            debug_output("update measured_card_table set card_status=1,used_times=$db_used_times where card_id='".$input_card_id."'");
            if($conn->errorCode() != '00000')
            {
