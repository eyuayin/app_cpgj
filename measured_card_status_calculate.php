...
<?php
    ...
    $query_result = $conn->query("select count(*) from measured_card_table_cui");
    debug_output("select count(*) from measured_card_table_cui");
    ...
    for($i = 0; $i < $loop_num; $i++)
    {
        ...
        $query_result = $conn->query("select valid_begin_date,valid_end_date,pause_begin_date,pause_end_date,card_id from measured_card_table_cui limit $start_num,$item_num");
        debug_output("select valid_begin_date,valid_end_date,pause_begin_date,pause_end_date,card_id from measured_card_table_cui limit $start_num,$item_num");
        ...
        foreach($result as $value)
        {
            ...
            //把卡状态值更新到数据库
            $update_result = $conn->query("update measured_card_table_cui set card_status=$card_status where card_id='".$card_id."'");
            debug_output("update measured_card_table_cui set card_status=$card_status where card_id='".$card_id."'");
        }
    }
?>