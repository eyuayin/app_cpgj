...
<?php
    ...
    /*if(($input_card_id == 1) || ($input_name == '体验号'))
    {
        ...
        {
            $query_result = $conn->query("select member_id from member_info_table_cui where card_id='".$input_card_id."'");
            $result = $query_result->fetch();
            $db_member_id = $result[0];
        }
        else if($book_mode == BOOK_MODE_NAME)   //按照姓名查询
        {
            $query_result = $conn->query("select member_id from member_info_table_cui where member_name='".$input_name."'");
            $result = $query_result->fetch();
            $db_member_id = $result[0];
        }
        ...
        //选课成功，将数据插入到数据库
        $query_result = $conn->query("insert into class_booking_table_cui values($input_class_id, $db_member_id, 0, 0)");
        debug_output("insert into class_booking_table_cui values($input_class_id, $db_member_id, 0, 0)");
        if($conn->errorCode() != '00000')
        {
            echo "数据库错误！预约失败！请重试！";
            return;
    ...
    {*/
        ...
        else if($book_mode == BOOK_MODE_NAME)   //按照姓名预约
        {
            ...
            debug_output("按照姓名预约！");
            $query_result = $conn->query("select member_id,card_type,card_id from member_info_table_cui where member_name='".$input_name."'");
            debug_output("select member_id,card_type from member_info_table_cui where member_name='".$input_name."'");
            $result = $query_result->fetch();
            $db_member_id = $result[0];
            debug_output("从DB中查出的member_id是".$db_member_id);
            $db_card_type = $result[1];
        ...
        {
            debug_output("计时卡");
            $query_result = $conn->query("select card_status,card_priority,used_times,max_times,next_used_times,concrete_card_type,valid_begin_date,valid_end_date,pause_begin_date,pause_end_date from time_card_table_cui where card_id='".$input_card_id."'");
            debug_output("select card_status,card_priority,used_times,max_times,next_used_times,concrete_card_type,valid_begin_date,valid_end_date,pause_begin_date,pause_end_date from time_card_table_cui where card_id='".$input_card_id."'");
            $result = $query_result->fetch();
        ...
        {
            debug_output("计次卡");
            $query_result = $conn->query("select card_status,card_priority,used_times,max_times from measured_card_table_cui where card_id='".$input_card_id."'");
            debug_output("select card_status,card_priority,used_times,max_times from measured_card_table_cui where card_id='".$input_card_id."'");
            $result = $query_result->fetch();
        ...
        if($db_card_type == TIME_CARD_TYPE) //计时卡
        {
            if($concrete_card_type == 8)    //不限次数卡
            {
                ...
                //在class_booking_table_cui中查询，如果有此member_id对应的class_id，则表示该会员已经已约过当天的课程，不能再预约其他课程
        ...
        {
            //对class_info_table_cui中的已选人数要累加
            $selected_user_number++;
            $query_result = $conn->query("update class_info_table_cui set selected_user_number=$selected_user_number where class_id=$input_class_id");
            debug_output("update class_info_table_cui set selected_user_number=$selected_user_number where class_id=$input_class_id");
            if($conn->errorCode() != '00000')
            ...
            else if($db_card_type == MEASURED_CARD_TYPE) //计次卡
            {
                ...
                {
                    //更新数据库
                    $query_result = $conn->query("update measured_card_table_cui set card_status=3, used_times=$db_used_times where card_id='".$input_card_id."'");
                    debug_output("update measured_card_table_cui set card_status=3, used_times=$db_used_times where card_id='".$input_card_id."'");
                }
                else
                {
                    //更新数据库
