...
$st = $conn->query("select * from personal_class_booking_table ORDER BY class_begin_time DESC");
...
foreach ($rs as $value) 
    {
		            $stt = $conn->query("select member_name from member_info_table where member_id=$value[0]");
					$name =$stt->fetchAll();
                    echo "<tbody>";
                    echo "<tr>";
