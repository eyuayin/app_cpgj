...
<head>
    ...
    <script src="FE_all/js/bootstrap.min.js"></script>
    <link href="FE_all/css/bootstrap.css" rel="stylesheet" />
    <link href="FE_all/css/site.css" rel="stylesheet" />
    <link href="FE_all/css/bootstrap-responsive.css" rel="stylesheet" />
    <!--[if lt IE 9]>
...
<div id="light" class="white_content">
    <div id="dituContent">
        <form action="modify_class.php" method="post"  onsubmit="return check_input_valid();">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <select style="line-height:35px;" id="class_location" name="class_location" class="dropdown-select"  required="required"> <option value="君子兰" >君子兰</option></select>
                </div>
            </div>
            <div class="control-group">
...
$st = $conn->query("select * from (select * from class_info_table order by class_begin_time)  as table_a where class_begin_time between '".$selected_date_begin."' and '".$selected_date_end."'");
debug_output("select * from (select * from class_info_table order by class_begin_time) as table_a  where class_begin_time between '".$selected_date_begin."' and '".$selected_date_end."'");
$rs = $st->fetchAll();
...
{
    //查出当前课程最大等待的编号
    //$query_result = $conn->query("select waiting_No,sign_in from class_booking_table where class_id=$value[0] order by waiting_No desc");
    ...
    //查出预约上课人数
    $q_result = $conn->query("select count(*) from class_booking_table where class_id=$value[0] and waiting_No=0 and canceled=0");
    ...
    //查出实际签到人数
    $q_result = $conn->query("select count(*) from class_booking_table where class_id=$value[0] and sign_in=1 and waiting_No=0 and canceled=0");
    debug_output("select count(*) from class_booking_table where class_id=$value[0] and sign_in=1 and waiting_No=0 and canceled=0");
    $q_value = $q_result->fetch();
    ...
    print <<<EOT
        ...
        <td>{$classroom_num_to_name[$value[5]]}</td>
        <td>{$value[7]}</td>
        <td>$db_booking_num</td>
        <td>$db_attendees_No</td>        
        <td>0</td>
        <td><a href="./FE_all/list_booked_members.php?begin_time=$value[2]&location=$value[4]&classroom=$value[5]&class_id=$value[0]">查看会员</a></td>
        <td><a href="./FE_all/delete_class.php?class_id=$value[0]" onClick="return confirm('确认要删除？')">删除</a></td>
...
<div class="row" style="margin-left:10px;">
    <a href="FE_all/new-role.php" class="btn btn-success col-md-6">预约课程</a>
    <a href="FE_all/search_and_update_class.php" class="btn btn-success col-md-6">预约查询</a>
</div>
</div>
</div>
</div>
